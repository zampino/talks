...
(function() {
  ...
  ApplicationController = (function() {
    ...
    function ApplicationController(scope, root) {
      this.scope = scope;
      this.root = root;
      this.saveState = __bind(this.saveState, this);
      scope.$on('$destroy', this.saveState);
    }

    ApplicationController.prototype.saveState = function() {
      console.log('///// saving state /////');
      alert('foo');
      return store.set(this.root.talkId, {
        slide: this.root.current
      });
    };
...
(function() {
  ...
  SlidesController = (function() {
    SlidesController.$inject = ['$location', '$rootScope', '$routeParams'];

    function SlidesController(location, root, params) {
      this.location = location;
      this.root = root;
      this.params = params;
      console.log('boot slides', params);
      root.talkId = params.talkId;
      root.current = parseInt(params.slide) || this.restoreState() || 0;
      root.keyPressed = (function(_this) {
        return function(e) {
          var reaction;
    ...
    SlidesController.prototype.controls = {
      key_37: function(location) {
        var target;
        target = this.root.current - 1;
        return this.location.path("" + this.params.talkId + "/" + target);
      },
      key_39: function(location) {
        var target;
        target = this.root.current + 1;
        return this.location.path("" + this.params.talkId + "/" + target);
      }
    ...
    SlidesController.prototype.restoreState = function() {
      var storedState;
      storedState = store.get(this.params.talkId) || {};
      return storedState.slide;
    };
...
(function() {
  Talks.directive('slide', function() {
    var linkFn;
    linkFn = function(scope, element, attributes) {
      ...
      updateCurrent = function(value) {
        ...
        return element.toggleClass('current', isCurrent);
      };
      return scope.$watch('current', function(newValue) {
        return updateCurrent(newValue);
      });
    };
    return {
      link: linkFn,
...
(function() {
  Talks.directive('slides', function() {
    var linkFn;
    linkFn = function(scope, element, attributes) {
      var slides;
      ...
      if (slides.length !== 0) {
        _.each(slides, function(slide, idx) {
          return angular.element(slide).data('index', idx);
        });
        return console.log('slides indexed', slides.length);
      }
    };
    return {
      link: linkFn,
